  ...
  return (
    <DropdownMenu.Portal>
      <DropdownMenu.Content
        ...
        sideOffset={6}
        className={cn(
          "z-50 min-w-48 rounded-lg border border-rule bg-card p-1 shadow-pop data-[state=open]:animate-fade-in",
          className,
        )}
        {...props}
      />
...
const itemClass =
  "flex cursor-pointer items-center gap-2 rounded-md px-2.5 py-1.5 text-[13px] text-ink-2 outline-none select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 data-[highlighted]:bg-sunken data-[highlighted]:text-ink [&_svg]:size-4 [&_svg]:text-muted";
...
export function MenuLabel({ className, ...props }: ComponentProps<typeof DropdownMenu.Label>) {
  return <DropdownMenu.Label className={cn("px-2.5 pt-1.5 pb-1 text-[11px] font-semibold text-faint uppercase tracking-[0.12em]", className)} {...props} />;
}
